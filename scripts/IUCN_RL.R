...
rm(list=ls())
setwd("D:/Work/oneSTOP") # adjust as needed
...
# 1455 assessments in Europe for species with EN category if i keep only the latest ones
...
# 1514 assessments in Europe for species with VU category if i keep only the latest ones


## 1.4 Get the assessment IDs for the species in Europe categorized as NT
rl_NT_assessments <- rl_categories(code = "NT",
                                   latest = T,
                                   scope_code = 2,
                                   key = IUCN_REDLIST_KEY)$assessments

length(rl_NT_assessments$assessment_id) 
# 1671 assessments in Europe for species with NT category if i keep only the latest ones
...
# here I use the same codes as in the IUCN report (Smith et al. 2022)
ias_codes <- c("8_1", "8_1_1", "8_1_2", 
               "8_4", "8_4_1", "8_4_2")

# function to extract the info needed from the assessments
clean_assessments <- function(x) {  
  
  # here, "x" (the subject of the function) is the assessment extracted through rl_assessment()
...
# for every category (CR, EN, VU, NT)
for(cat in c("CR", "EN", "VU", "NT")){ 
