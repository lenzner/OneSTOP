...
# Started on 12.02.2026
# modified on 02.03.2026 
# modified on 28.03.2026
...
message("========= Processing species ", sp, " for global climate model with pseudo-presences")
...
# check if the species has already been modelled
...
if(file.exists(global_model_file)){ 
  message("Climate model already exists for ", speciesName, ". Skipping.")
...
message(paste0("Cleaned number of pseudo-presences for ", sp, ": ", nrow(global.occ.sf)))
...
if(class(wwf_ecoSub1)[1] != "SpatVector"){wwf_ecoSub1_vector <- vect(wwf_ecoSub1) 
...
# NB The original wiSDM2.0 samples pseudo-absences in cells that are not presences,
# (what they call "uncertain absences" in Si-Moussi et al. 2025)
# but does not account for IUCN ranges 
# in my case, i should sample ONLY outside the IUCN range
# (what they call "certain absences" in Si-Moussi et al. 2025)

if(file.exists(paste0("./data/processed/IUCN_sp_shp/single_sp_rast_cropped_", project_version, "/", sub(" ", "_", sp), ".tif"))){
  
  # load sp raster
  sp_rast <- terra::rast(paste0("./data/processed/IUCN_sp_shp/single_sp_rast_cropped_", project_version, "/", sub(" ", "_", sp), ".tif"))
  # reproject
  sp_rast <- terra::project(sp_rast, project_projection)
  # resample
  sp_rast <- terra::resample(sp_rast, biasgrid_sub, method = "near")
  # mask values of sp_rast
  biasgrid_sub <- terra::mask(biasgrid_sub, sp_rast, inverse = TRUE)
  
}else{
  warning("Can't exclude IUCN range cells for pseudo-absences sampling, because no IUCN rasterized range is available for ", sp)}
...
cat("Success! Climate model with pseudo-presences has been created for", sp, "in", round(elapsed, 2), "hours\n\n")
rm(list = ls())
