...
message("========= Processing species ", sp, " for European habitat model with GBIF presences")
...
eu_occ <- tryCatch({
  ...
  # depending on the data, the columns may be X/Y or decimalLongitude/lat
  # in any case, they are always in projectprojection, irregardless of the column name
  
  cols_eu_occ <- colnames(eu_occ)
  
  if("X" %in% cols_eu_occ){cols_eu_occ <- colnames(eu_occ)
  }else if("decimalLongitude" %in% cols_eu_occ){
      cols_eu_occ <- colnames(eu_occ)}
  ...
  if("X" %in% cols_eu_occ){names(occ_vect) <- c("decimalLongitude", "decimalLatitude")
  message("Column names for duplicate removal are renamed lon/lat")}
...
if (nrow(eu_occ) < 20) {
  warning(paste(nrow(eu_occ)," occurrences in Europe for species:", species, 
                "\n- European model cannot be constructed"))
  stop()
}
...
# Load WWF ecoregions
#wwf_ecoregions <- sf::st_read(paste0(database_path, ("/wwf_terrestrial_ecoregions/data/commondata/data0/wwf_terr_ecos.shp")))
...
cat("Success! Habitat and ensemble models with GBIF presences have been created for", sp, "in", round(elapsed, 2), "hours\n\n")
rm(list = ls())
