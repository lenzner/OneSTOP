...
message("========= Processing species ", sp, " for CV validation with GBIF presences")
...
# message("Starting number of pseudo-presences for ", sp, ": ", nrow(split_df))
...
if (use_cv || use_cv_climate_only) {
  ...
  # make sure there is an ID column, otherwise extract_env() fails
  global_presabs <- global_presabs %>%
    dplyr::mutate(ID = dplyr::row_number())
  ...
  if(!use_cv_climate_only){
    ...
    if(eu_climate_validation){
      ...
      warning("No cross-validation for the climate model can be performed, because ", sp, " has less than 40 presences in Europe.")
    }
  }
...
final_validation$project_name <- projectname
...
cat("Success! Spatial CV validation for model with GBIF presences has been created for", sp, "in", round(elapsed, 2), "hours\n\n")
